...
-- 3. TASA DE LETALIDAD POR COMPLICACION
WITH casos_confirmados_complicacion AS (
    SELECT
        ...
        END AS fallecido
    FROM raw.paciente
    LEFT JOIN raw.resultado
        ON resultado.paciente_id = paciente.id
    LEFT JOIN raw.paciente_complicacion
        ON paciente_complicacion.paciente_id = paciente.id
    LEFT JOIN raw.complicacion
        ON complicacion.id = paciente_complicacion.complicacion_id
...
WITH casos_confirmados_condicion AS (
    ...
    LEFT JOIN raw.resultado
        ON resultado.paciente_id = paciente.id
    LEFT JOIN raw.paciente_condicion
        ON paciente_condicion.paciente_id = paciente.id
    LEFT JOIN raw.condicion
        ON condicion.id = paciente_condicion.condicion_id
    WHERE resultado.clasificacion_final IN (
        'CASO DE COVID-19 CONFIRMADO POR ASOCIACIÓN CLÍNICA EPIDEMIOLÓGICA',
        'CASO DE COVID-19 CONFIRMADO POR COMITÉ DE  DICTAMINACIÓN',
        'CASO DE SARS-COV-2 CONFIRMADO'
